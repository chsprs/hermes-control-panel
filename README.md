...
- 📡 **Manajemen & Konfigurasi Gateway Perpesanan**:
  ...
  - **Log Gateway & WhatsApp Bridge Realtime**: Log aktivitas gateway Hermes (`gateway.log` & journalctl) dan jembatan WhatsApp (`bridge.log`) dapat dipantau langsung di Tab Layanan dengan toggle switcher tab (`Gateway Core` vs `WhatsApp Bridge`), live via SSE dengan auto-scroll, tombol sembunyikan/tampilkan, dan proteksi sensor token otomatis (`[REDACTED]`). Dilengkapi endpoint `GET /api/gateway-log` dan `GET /api/whatsapp-log`.
  - **Pairing WhatsApp QR Code Langsung**: Fitur pairing WhatsApp Baileys interaktif langsung dari Web UI tanpa perlu terminal SSH. Menampilkan modal pairing, render vector SVG QR code yang tajam, terminal log proses pairing live, deteksi koneksi otomatis, dan tombol aktivasi instan ke `config.yaml`. Didukung endpoint `/api/whatsapp/pair-start`, `/api/whatsapp/pair-status`, `/api/whatsapp/pair-cancel`, dan `/api/whatsapp/pair-apply`.
